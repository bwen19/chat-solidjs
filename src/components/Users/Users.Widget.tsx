...
import { Modal, EditSolid } from "../common";
...
export const CreateUserButton: Component<{ reload: Accessor<void> }> = (props) => {
  ...
  return (
    <button onClick={[setOpen, true]} class="rounded-md border bg-sky-700 px-3 py-1.5 text-sm text-white hover:bg-sky-600 active:bg-sky-700">
      Add user
      <Show when={open()}>
        <CreateUserModal reload={props.reload} onClose={() => setOpen(false)} />
      </Show>
    </button>
  );
};

const CreateUserModal: Component<{ reload: Accessor<void>; onClose: Accessor<void> }> = (props) => {
...
export const EditUserButton: Component<{ user: UserInfo; reload: Accessor<void> }> = (props) => {
  ...
  return (
    <div onClick={[setOpen, true]}>
      ...
      <Show when={open()}>
        <EditUserModal user={props.user} reload={props.reload} onClose={() => setOpen(false)} />
      </Show>
    </div>
  );
};
const EditUserModal: Component<{ user: UserInfo; reload: Accessor<void>; onClose: Accessor<void> }> = (props) => {
