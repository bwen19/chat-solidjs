...
import { Avatar, Modal, SearchBox, UserAddOutline } from "../common";
import { useFriend, useFindUser } from "./friend.service";

export const AddFriendModalWrapper: ParentComponent = (props) => {
  ...
  return (
    <div onClick={[setOpen, true]}>
      {props.children}
      <Show when={open()}>
        <AddFriendModal onClose={() => setOpen(false)} />
      </Show>
    </div>
  );
};

const AddFriendModal: ParentComponent<{ onClose: Accessor<void> }> = (props) => {
